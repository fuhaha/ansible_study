...
- PlayBook Include : 하나의 플레이에서 다른 파일의 플레이를 포함할 때 사용
  ...
  usersetup.yml : 하나의 사용자를 생성하는 Task
      ---
  ...
  multiple-usersetup.yml : user 변수를 넘겨주어 다수의 사용자를 생성
      ---
      - hosts: ansibletest
        user: root
        tasks:
  ...
  sendmailhandlers.yml : sendmail 설정 후 처리하기를 원하는 공통 Task를 Handler 파일로 제공
      ---
  ...
  update_config_sendmail.yml : sendmail 최신버전으로 업데이트 및 config 파일을 Template으로 교체
      ---
        hosts: mailers															#1
        tasks:																	#2
  ...
  site_upgrade.yml : 사이트 전체를 업그레이드 하는 PlayBook
      ---
...
5. common, apache, website1, website2 Role을 사용하고자 한다면 다음 예제와 같은 디렉토리 구조로 생성
   ...
   website1.yml : common, apache, website1 Role을 website1 그룹 장비에 적용하는 PlayBook
       ---
...
webservers.yml : patch, deploy, config Tag를 정의
